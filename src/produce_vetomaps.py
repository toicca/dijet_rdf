...
def produce_vetomap(input_file: str, trigger_list: List[str], output_path: str):
    ...
    # Get the folder name from hist_info
    systems = [h[0] for h in hist_info]
    methods = [h[1] for h in hist_info]
    hist_names = [h[2] for h in hist_info]
    ...
    for trg in trigger_list:
        ...
        h2pulls = ROOT.TH2D("h2pulls", "Pulls", bins["eta"]["n"], bins["eta"]["bins"], 300, -3, 3)
        ...
        for system, method, hname in hist_info:            
            ...
            for i in range(1, h2.GetNbinsX()+1):
                deltas = ROOT.TH1D("deltas", "Distribution of values in an eta strip", 300, -3, 3)
...
def produce_vetomap2(input_file: str, trigger_list: List[str], output_path: str):
    """
    VetoMap producer for dijet_rdf.
    Translated from https://github.com/miquork/jecsys3/blob/21fcfb6c4a6fab963650b08019f57d679644ec83/minitools/doJetVetoV2.C
    """
    ...
    for trg in trigger_list:
            ...
            if not file.GetDirectory(trg):
                file.mkdir(trg)
            if not file.GetDirectory(trg+"/standard/VetoMap2"):
                file.mkdir(trg+"/standard/VetoMap2")
            file.cd(trg+"/standard/VetoMap2")
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        produce_vetomap2(file, trigger_list, output_path)
