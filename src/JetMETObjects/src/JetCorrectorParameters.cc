...
//
// #include "macros/common_info/JetMETObjects/interface/Workarounds.h"
...
JetCorrectorParameters const & JetCorrectorParametersCollection::operator[]( key_type k ) const {
  ...
  std::cerr << " cannot find key " << static_cast<int>(k) << " in the JEC payload, this usually means you have to change the global tag" << std::endl;
  throw runtime_error("error in JetCorrectorParametersCollection::operator[]");
}
...
JetCorrectorParametersCollection::findKey( std::string const & label ) const {
  ...
  throw runtime_error("error in JetCorrectorParametersCollection::findKey");
}
