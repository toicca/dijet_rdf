...
#include "../interface/Workarounds.h"
#include <vector>
#include <string>
...
float SimpleJetCorrectionUncertainty::uncertaintyBin(unsigned fBin, float fY, bool fDirection) const 
{
  ...
  if ((p.size() % 3) != 0){
    cerr << "wrong # of parameters: multiple of 3 expected, "<<p.size()<< " got";
    throw runtime_error("error in SimpleJetCorrectionUncertainty::uncertaintyBin");
  }
