...
def add_produce_time_evolution_parser(state):
    ...
    time_evolution_parser = subparsers.add_parser(
        "produce_time_evolution",
        help="Produce time \
            evolution for given input files",
    )
    time_evolution_files = time_evolution_parser.add_mutually_exclusive_group(
        required=True
    )
    time_evolution_files.add_argument(
        "--filelist",
        type=str,
    ...
    time_evolution_parser.add_argument(
        "-tf",
        "--triggerfile",
        type=str,
    ...
    time_evolution_parser.add_argument(
        "-ch", "--channel", type=str, help="Channel to be used", choices=state.channels
    )
    time_evolution_parser.add_argument(
        "--out",
        type=str,
...
def data_hists(rdf, hist_config, bins):
    ...
    for hist in hist_config:
        ...
        if hist_config[hist]["type"] == "Histo1D":
            ...
            if cut:
                hd[hist] = rdf.Filter(cut).Histo1D(
                    (name, title, bins[x_bins]["n"], bins[x_bins]["bins"]),
                    x_val,
                    "weight",
                )
            else:
            ...
            cut = hist_config[hist].get("cut")
            if cut:
                hd[hist] = rdf.Filter(cut).Profile1D(
                    (name, title, bins[x_bins]["n"], bins[x_bins]["bins"]),
                    x_val,
                    y_val,
                    "weight",
...
def produce_time_evolution(lds, hist_config, bins, logging):
    ...
    for hist in hist_config:
        ...
        hs[hist] = ROOT.TH1D(
            f"{name}_int_lumi", title, len(lumi_bins) - 1, np.array(lumi_bins)
        )
    ...
    logging.info(
        f"Done filling time evolution histograms (took {time.time() - start_time_evolution} s)"
    )
...
def run(state):
    ...
    if args.data_tag:
        output_path = (
            f"{args.out}/J4PTimeEvolution_runs{min_run}to{max_run}_{args.data_tag}.root"
        )
    else:
