...
def add_vetomaps_parser(subparsers):
    ...
    vetomaps_triggers.add_argument(
        "--triggerlist",
        type=str,
        help="Comma separated list of \
...
def produce_vetomap_old(input_file: str, trigger_list: List[str], output_path: str):
    ...
    for trg in trigger_list:
        ...
        h2pulls = ROOT.TH2D(
            "h2pulls", "Pulls", bins["eta"]["n"], bins["eta"]["bins"], 100, -3, 3
        )
        h2pullmap = ROOT.TH2D(
            "h2pullmap",
            "Pull map",
        ...
        h1ns = ROOT.TH1D(
            "h1ns",
            "Number of non-empty bins in an eta strip",
            bins["eta"]["n"],
            bins["eta"]["bins"],
        )
        ...
        h2nomrefsum = h2nomsum.Clone(
            "h2nomrefsum"
        )  # nomrefsum is the information from asymmetry plots
        ...
        for system, method, hname in hist_info:
            ...
            assert obj.InheritsFrom(
                "TH2D"
            ), f"Object at {obj_path} is not a TH2D or derived class"
                ...
                for i in range(1, h2.GetNbinsX() + 1):
                    for j in range(1, h2.GetNbinsY() + 1):
                        if h2.GetBinError(i, j) != 0:
                            if h2.GetBinError(i, j) != 0:
                                val1, err1 = h2.GetBinContent(i, j), h2.GetBinError(
                                    i, j
                                )
                                n1 = 1.0 / pow(err1, 2)
                                val2, err2 = h2.GetBinContent(i, j), h2.GetBinError(
                                    i, j
                                )
                                n2 = 1.0 / pow(err2, 2)
            ...
            for i in range(1, h2.GetNbinsX() + 1):
                deltas = ROOT.TH1D(
                    "deltas", "Distribution of values in an eta strip", 100, -3, 3
                )
                ...
                if projection.GetNbinsX() % 2 == 0:
                    median = (
                        projection.GetBinContent(projection.GetNbinsX() // 2)
                        + projection.GetBinContent(projection.GetNbinsX() // 2 + 1)
                    ) / 2
                else:
        ...
        for i in range(1, h2nomsum.GetNbinsX() + 1):
            for j in range(1, h2nomsum.GetNbinsY() + 1):
                ...
                elif (h1ns.GetBinContent(i) > 70) and (
                    h2nomref.GetBinContent(i, j) < 0
                ):
                    h2veto.SetBinContent(i, j, 100)
...
def produce_vetomap(input_file: str, trigger_list: List[str], output_path: str):
    ...
    for trg in trigger_list:
        ...
        veto_map_hotandcold = ROOT.TH2D(
            "veto_map_hotandcold",
            "Veto map hot and cold",
            bins["eta"]["n"],
            bins["eta"]["bins"],
            bins["phi"]["n"],
            bins["phi"]["bins"],
        ...
        for system, method, hname in hist_info:
            ...
            for i in range(1, h2.GetNbinsX() + 1):
                ...
                for j in range(1, h2.GetNbinsY() + 1):
                    sum_of_methods.SetBinContent(
                        i,
                        j,
                        sum_of_methods.GetBinContent(i, j) + h2.GetBinContent(i, j),
                    )
                    sum_of_methods.SetBinError(
                        i, j, sum_of_methods.GetBinError(i, j) + h2.GetBinError(i, j)
                    )

        # Normalize the sum of methods to strip mean = 1
        sum_of_methods.Scale(1.0 / len(hist_info))
        ...
        for i in range(1, sum_of_methods.GetNbinsX() + 1):
            ...
            if sigma != 0:
                for j in range(1, sum_of_methods.GetNbinsY() + 1):
                    ...
                    if median != 0:
                        if (sum_of_methods.GetBinError(i, j) != 0) and (
                            sum_of_methods.GetBinError(i, j) / median < 0.5
                        ):
                            delta = (
                                sum_of_methods.GetBinContent(i, j) - median
                            ) / sigma
                            veto_map_loose.SetBinContent(
                                i, j, 0 if ((delta < 5) and (delta > -5)) else 1
                            )
                            veto_map_medium.SetBinContent(
                                i, j, 0 if ((delta < 3) and (delta > -3)) else 1
                            )
                            veto_map_tight.SetBinContent(
                                i, j, 0 if ((delta < 1) and (delta > -1)) else 1
                            )
