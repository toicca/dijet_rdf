...
def init_zee(rdf, jet_columns, state):
    ...
    rdf = (rdf.Define("electronMask", "Electron_pt > 15 && Electron_cutBased >= 3 && Electron_pfRelIso03_all < 1.0")
            ...
            .Filter("goodElectron_pt.size() > 1 && goodElectron_pt.size() < 4", "2-3 tight electrons")
    )

    # Trigger selected electrons
    rdf = (rdf.Define("trigMask", "hasTrgObj(goodElectron_eta, goodElectron_phi, TrigObj_eta, TrigObj_phi, TrigObj_filterBits, TrigObj_id)")
            ...
            .Filter("selElectron_pt.size() > 1 && selElectron_pt.size() < 4", "2-3 tight electrons with trigger match")
            .Define("Electron_idx_temp", "findElectronIdxs(selElectron_pt, selElectron_eta, selElectron_phi, selElectron_mass, selElectron_charge)")
