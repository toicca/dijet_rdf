...
def init_photonjet(rdf, jet_columns, state):
    ...
    rdf = (rdf.Define("hasTrg_temp", "hasTrgObj(goodPhoton_eta, goodPhoton_phi, TrigObj_eta, TrigObj_phi, TrigObj_id)")
            ...
            .Define("Tag_rawFactor_temp", "selPhoton_seedGain[Tag_idx_temp] == 1 ? 1./1.017 : 1")
    ...
    # Probe jet
    rdf = (rdf.Define("Jet_indices_temp", "findJetIdxs(Jet_pt, Jet_eta, Jet_phi, goodPhoton_eta, goodPhoton_phi, goodPhoton_jetIdx)")
            .Define("Probe_idx_temp", "Jet_indices_temp.first")
            .Filter("Probe_idx_temp >= 0",
                    "Jet found")
            .Filter("Jet_pt[Probe_idx_temp] > 15", 
                    "Jet pT > 15")
    ...
    rdf = (rdf.Define("metJetIdx_temp", "metJetIdx(Jet_pt, Jet_eta, Jet_phi, Tag_eta, Tag_phi)")
                ...
                .Define("metJet_rawFactor_temp", "ROOT::VecOps::Take(Jet_rawFactor, metJetIdx_temp)")
                .Define("metJet_polVec_temp", "ROOT::VecOps::Construct<ROOT::Math::Polar2DVector>(metJet_pt_temp, metJet_phi_temp)")
                .Redefine("metJet_polVec_temp", "ROOT::VecOps::Sum(metJet_polVec_temp, ROOT::Math::Polar2DVector())")
                .Define("metJet_rawPolVec_temp", "ROOT::VecOps::Construct<ROOT::Math::Polar2DVector>((1.0 - metJet_rawFactor_temp) * metJet_pt_temp, metJet_phi_temp)")
                .Redefine("metJet_rawPolVec_temp", "ROOT::VecOps::Sum(metJet_rawPolVec_temp, ROOT::Math::Polar2DVector())")
                .Define("PhT1MET_polar_temp", "ROOT::Math::Polar2DVector(RawPuppiMET_pt, RawPuppiMET_phi)")
                .Redefine("PhT1MET_polar_temp", "PhT1MET_polar_temp - metJet_polVec_temp + metJet_rawPolVec_temp - RawPhTag_polVec_temp + PhTag_polVec_temp")
    ...
    # PhotonJet for cuts
    rdf = (rdf.Define("PhotonJet_temp", "selPhoton_jetIdx[0] < 0 ? ROOT::Math::Polar2DVector(0, 0) : \
                ROOT::Math::Polar2DVector(Jet_pt[selPhoton_jetIdx[0]], Jet_phi[selPhoton_jetIdx[0]])")
           .Define("PhotonJet_raw_temp", "(1.0 - Jet_rawFactor[selPhoton_jetIdx[0]]) * PhotonJet_temp")
           )

    rdf = (rdf.Filter("fabs(1.0-Probe_pt / Tag_pt) < 0.7",
                      "|1-DB| < 0.7; gamjet pass_bal")
                .Filter("fabs(PhT1MET_polar_temp.Dot(PhTag_polVec_temp)/(Tag_pt*Tag_pt)) < 0.7",
                        "|1-MPF| < 0.7; gamjet pass_mpf")
                .Filter("selPhoton_jetIdx[0] >= 0 ? \
                        ROOT::VecOps::DeltaR(Jet_eta[selPhoton_jetIdx[0]], Tag_eta, Jet_phi[selPhoton_jetIdx[0]], Tag_phi) < 0.4 : 1",
                        "PhotonJet dR < 0.4")
                .Filter("selPhoton_jetIdx[0] >= 0 ? (PhotonJet_raw_temp - RawPhTag_polVec_temp).R() < 0.06*Tag_pt : 1",
