...
def init_zmm(rdf, jet_columns, state):
    ...
    rdf = (rdf.Define("trigMask", "hasTrgObj(goodMuon_eta, goodMuon_phi, TrigObj_eta, TrigObj_phi, TrigObj_filterBits, TrigObj_id)")
            ...
            .Filter("selMuon_pt.size() > 1 && selMuon_pt.size() < 4", "2-3 tight muons with trigger match")
            ...
            .Filter("selMuon_pt[Muon_idx_temp.first] > 20 && selMuon_pt[Muon_idx_temp.second] > 10", "Leading muon pT > 20, subleading muon pT > 12")
            .Filter("abs(selMuon_eta[Muon_idx_temp.first]) <= 2.3 && abs(selMuon_eta[Muon_idx_temp.second]) <= 2.3", "Muon eta <= 2.3")
    )
    ...
    # Probe jet selection
    rdf = (rdf.Define("JetMuon_idx_temp", "findJetIdxs(Jet_eta, Jet_phi, goodMuon_eta, goodMuon_phi)")
            ...
            .Filter("(1.0-Jet_rawFactor[Probe_idx_temp])*Jet_pt[Probe_idx_temp] > 12", "Leading jet raw pT > 12")
